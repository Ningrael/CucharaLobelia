 ...
 */
function buildGroundedContext(queryText) {
  if (!rulesKnowledge || rulesKnowledge.length === 0) {
    return '';
  }
  ...
  const faqs = rulesKnowledge.filter(doc => doc.category === 'FAQ & Erratas');

  // 3. REGLAS DE EJÉRCITO Y PERFILES ESPECÍFICOS:
  const nonCorePages = rulesKnowledge.filter(doc => doc.category !== 'Reglamento Principal' && doc.category !== 'FAQ & Erratas');
...
export async function askRulesAi(input, customApiKey = '', conversationHistory = [], lang = 'es') {
  ...
  const contextSnippet = buildGroundedContext(queryText);

  if (!contextSnippet) {
    throw new Error('Base de conocimiento de reglas no disponible.');
  }
